...
<template>
    <Teleport to=".movie--trailer">
      <div
        class="bg-black bg-opacity-50 blur-2xl fixed top-0 h-full z-50 w-full flex items-center justify-center"
      >
        <div class="bg-black bg-opacity-90 w-full max-w-2xl h-auto p-4 rounded-lg">
          <ModalLogo @close="$store.commit('showTrailer', false)" />
          <template v-if="getRandomTrailer !== []">
            <div class="flex items-center justify-between">
              <h2 class="text-white text-3xl font-medium my-3">
                {{ getRandomTrailer?.name }}
              </h2>
              <button
                v-if="trailer.state.trailers.length >= 1"
                @click="setRandomTrailer"
            ...
            <div>
              <YouTube
                v-if="getRandomTrailer !== null"
                :src="`https://www.youtube.com/watch?v=${getRandomTrailer?.key}`"
                @ready="onReady"
                @error="errorWhileTryingTOPlay"
                ref="youtube"
